...
> **v1.4.1 管线顺序**：computed 在 belongsTo 关系填充 FK 之后求值，因此 computed 字段可以引用 relation 填充的 FK 字段（如 `coalesce(storeId, 0)`）。求值顺序：`makeN → applyRelations → applyComputed → applyNullable → collectIds`。computed 按字段声明顺序求值，依赖项需先声明。

### 场景化生成（v1.4.1）

`generateFromSpec(spec, { scenario })` 按 `spec.scenarios[scenario]` 覆盖实体 count（生成期）；未提及的实体保持默认 count：

```json
{
  "scenarios": {
    "default": { "PaymentLog": { "count": 50 }, "StoreInfo": { "count": 5 } },
    "empty":   { "PaymentLog": { "count": 0 },  "StoreInfo": { "count": 0 } },
    "minimal": { "PaymentLog": { "count": 3 },  "StoreInfo": { "count": 1 } }
  }
}
```
...
```typescript
const spec = parseMockDataSpec(jsonString);
const minimal = generateFromSpec(spec, { scenario: "minimal" });  // PaymentLog=3, StoreInfo=1
const empty   = generateFromSpec(spec, { scenario: "empty" });    // 全部 0 条（验证空态 UI）
```

- **未知场景** → 抛错，错误信息列出可用场景名
- **count=0** → 实体返回 `[]`，依赖它的子实体 relation 跳过 FK 填充（warn）
- **运行时场景切换**（数据+注入联动、不重新生成）仍用 `createScenarioContext`——两者正交：`generateFromSpec` 场景参数管"生成什么量"，`createScenarioContext` 管"运行时行为"
...
| **v1.4.2** | **接收方导入工作流全套支持**：`buildDataset(seed, { unknownTables })` 未知表名检测（warn/error/ignore）+ skill Step 5 接收方导入（DatasetSeed / MockDataSpec / MOCK_SPEC.md 三路径）+ 共享与移植指引 | ✅ |
| **v1.4.3** | **gen-seed CLI + session handlers 补齐 + importDatasetSeed 透传**：`npx gen-seed --spec mock-data-spec.json` 一键导出 seed.json + `defaultSessionHandlers` 补 loginBySms/ping/registerSecure/loginByQrCode + 修复 loginByPassword/loginByContext 补 success + importDatasetSeed 透传 unknownTables | ✅ 当前版本 |
